    ...
    def place_orders(self, bid: int, ask: int) -> None:
        ...
        if bid and ask:
            ...
            if len(self.avg_20_list) > 0:
                ...
                if self.size > 0:
                    print(f"self.avg_20_list[-1] {self.avg_20_list[-1]}")
                    print(f"self.mark_to_market(self.holdings) {self.mark_to_market(self.holdings)}")
                    print(f"0.1*self.starting_cash {0.1*self.starting_cash}")
                    ...
                    print(f"self.holdigs {self.holdings}")
                    ...
                    if self.get_holdings(self.symbol) > 0 and (self.mark_to_market(self.holdings) > self.take_profit_trigger*self.last_mark_to_market or self.mark_to_market(self.holdings) < self.stop_loss_trigger*self.starting_cash):
                        ...
                        print("SA selling")
                        print(f"last_mark_to_market {self.last_mark_to_market}")
                        print(f"self.mark_to_market(self.holdings) {self.mark_to_market(self.holdings)}")
                    ...
                    elif self.avg_20_list[-1] >= self.max_buy_trigger:
                        print("-------------SA buying")
                        print(f"self.avg_20_list[-1] { self.avg_20_list[-1]}")
                        print(f"self.max_buy_trigger {self.max_buy_trigger}")
                        print(f"self.current_time {self.current_time}")
