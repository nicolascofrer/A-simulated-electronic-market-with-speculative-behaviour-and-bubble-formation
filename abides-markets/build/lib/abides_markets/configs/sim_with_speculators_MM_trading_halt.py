...
def build_config(
    ...
    num_speculative_agents=400,
    agents_wake_up_freq=15, #str_to_ns("30s")
    trading_halt_start=500,
    trading_halt_length=100,
    ...
    print(f"build version 16-01-2024 with MM trading halt with seed {seed}")
    # fix seed
    np.random.seed(seed)
    ...
    trading_halt_start = DATE + trading_halt_start
    ...
    agents.extend(
        [
            AdaptiveMarketMakerAgent(
                ...
                trading_halt_allowed=True,
                trading_halt_start=trading_halt_start,
                trading_halt_length=trading_halt_length,
            )
