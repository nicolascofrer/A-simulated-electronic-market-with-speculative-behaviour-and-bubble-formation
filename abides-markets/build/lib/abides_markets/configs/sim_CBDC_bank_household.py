...
def build_config(
    ...
    ticker="ASSET",
    starting_cash=100,  # Cash in this simulator is always in CENTS.
    ...
    r_bar=1.02,  # true mean fundamental value
    ...
    sigma_s=.01,
    fund_vol=10e-10,  # Volatility of fundamental time series.
    #megashock_lambda_a=1e-10,#2.77778e-18,
    megashock_lambda_a=3e-10,#2.77778e-18,
    ...
    mm_cancel_limit_delay=1,  # 50 nanoseconds
    # 5) Momentum Agents
    num_momentum_agents=0,
    # num_momentum_agents=100,

    num_speculative_agents=0,
    agents_wake_up_freq = 5 #str_to_ns("30s")
    ...
    print(f"build version 20-07-2023 with seed {seed}")
    # fix seed
    np.random.seed(seed)
    ...
    agents.extend(
        [
            CentralBank(
                id=j,
                name="CENTRAL_BANK{}".format(j),
                type="Central_Bank_agent",
                symbol=ticker,
                starting_cash=100e6,
                # pov=MM_PARAMS[idx][1],
                # min_order_size=MM_PARAMS[idx][4],
    ...
    agent_count += NUM_MM
    agent_types.extend("CentralBank")
    ...
    agents.extend(
        [
            CommercialBank(
                id=j,
                name="COMMERCIAL_BANK{}".format(j),
                type="Commercial_Bank_agent",
                symbol=ticker,
                starting_cash=100e6,
                # pov=MM_PARAMS[idx][1],
                # min_order_size=MM_PARAMS[idx][4],
    ...
    agent_count += NUM_MM
    agent_types.extend("CommercialBank")
