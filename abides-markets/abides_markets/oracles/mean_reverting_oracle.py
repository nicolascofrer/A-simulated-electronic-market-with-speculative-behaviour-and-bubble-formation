...
class MeanRevertingOracle(Oracle):
    ...
    def __init__(
        ...
        print(f"ORACLE r {self.r[symbol]}")
    ...
    def generate_fundamental_value_series(
        self, symbol: str, r_bar: int, kappa: float, sigma_s: float, mean_s: float
    ) -> pd.Series:
        ...
        s = pd.Series(index=date_range.astype(int)) # NC: Needed otherwise the date time does not match the int timestamp type

        r = np.zeros(len(s.index))
        r[0] = r_bar
        
        # Predetermine the random shocks for all time steps (at once, for computation speed).
        shock = np.random.normal(loc=mean_s, scale=sigma_s, size=(r.shape[0]))
        
        # NC: Here is the bimodal shock
        shock_sign = np.random.randint(2, size=(r.shape[0])) == 0

        # NC: Here just changing the sign of the shock
        shock = shock*shock_sign + shock*(shock_sign-1)
