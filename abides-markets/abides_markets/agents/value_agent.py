...
class ValueAgent(TradingAgent):
    def __init__(
        ...
        self.kappa: float = self.random_state.uniform() * kappa  # mean reversion parameter
        self.news_reaction: float = self.random_state.uniform() * news_reaction
        ...
        self.conditional_on_PnL_leverage: bool = conditional_on_PnL_leverage
        print(f"kappa {self.kappa}")
        print(f"reaction to news {self.news_reaction}")
    ...
    def updateEstimates(self) -> int:
        ...
        self.r_bar = obs_t # NC: The belief on the fundamental value
        ...
        self.r_t = (1-self.news_reaction) * r_tprime + self.news_reaction * obs_t
        ...
        # need this to log the MtM
        self.mark_to_market(self.holdings)
    ...
    def placeOrder(self) -> None:
        ...
        if ask and (self.conditional_on_PnL_leverage) and (self.mark_to_market(self.holdings) < self.starting_cash) and (buy == 1):
            self.size = min(self.size, int(self.holdings["CASH"]/ask) )

        side = Side.BID if buy == 1 else Side.ASK

#        if self.size > 0:
        if self.size > 0 and p>0:
            self.place_limit_order(self.symbol, self.size, side, p)
         #   if buy==1:
          #      self.place_limit_order(self.symbol, self.size, side, p)
          #  elif min(self.get_holdings(self.symbol),self.size) > 0:
          #      self.place_limit_order(self.symbol, min(self.get_holdings(self.symbol),self.size), side, p)
