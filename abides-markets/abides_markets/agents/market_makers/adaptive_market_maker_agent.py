...
class AdaptiveMarketMakerAgent(TradingAgent):
    ...
    def __init__(
        ...
        min_imbalance=0.9,
        trading_halt_allowed=False,
        trading_halt_start=np.inf,
        trading_halt_length=0,
    ) -> None:
        ...
        self.trading_halt_allowed: bool = trading_halt_allowed

        self.trading_halt_history: [] = []

        self.end_trading_halt_time: int = 0
        self.trading_halt_start: int = trading_halt_start
        self.trading_halt_length: int = trading_halt_length
    ...
    def compute_orders_to_place(self, mid: int) -> Tuple[List[int], List[int]]:
        ...
        if self.price_skew_param is None:
            ...
            # the agent skews using the inventory

            holdings = self.get_holdings(self.symbol)
            if holdings == 0: mid_point = mid

            elif holdings>0:

                mid_point = int(mid - self.price_skew_param)
                ...
                logger.debug(
                "Skewing mid by {}",
                holdings*self.price_skew_param
            )
            # buy_transacted_volume = self.transacted_volume[self.symbol][0]
            # sell_transacted_volume = self.transacted_volume[self.symbol][1]
    ...
    def place_orders(self, mid: int) -> None:
        ...
        # Here we add the option for a trading halt
        if not self.trading_halt():
            ...
            print(f"Trading halt, filtering out selling orders")
            orders = [order for order in orders if order.side.is_bid()]
    ...
    def get_wake_frequency(self) -> NanosecondTime:
        if not self.poisson_arrival:
            ...
            print(f'MM delta_time is {delta_time}')
            return int(round(delta_time))

    def trading_halt(self) -> bool:
    
        # clean_mids = [x for x in self.mid_list if x is not None]
        # max_mid = np.max(clean_mids)
        # min_mid = np.min(clean_mids)

        
        if not self.trading_halt_allowed: return False
        
        # rolling_vol = np.std(np.diff(np.log(clean_mids[-rolling_window_size:])))

        # print(f"clean_mids {clean_mids}")
        # print(f"rolling_vol {rolling_vol}")

        if self.current_time>self.trading_halt_start and self.current_time<self.trading_halt_start + self.trading_halt_length:
            # self.logEvent("TRADING HALT", f"rolling vol {rolling_vol} last mid {clean_mids[-1]}")

            self.trading_halt_history.append({'time':self.current_time, 'mid':self.mid_list[-1]})

            # self.end_trading_halt_time = self.last_time_book_order + trading_halt_length
            return True
